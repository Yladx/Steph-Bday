document.addEventListener("DOMContentLoaded", function(){
	...
	function setStaticPosition() {
		...
		TweenMax.set(loadingText, {opacity: 1, y: 0});
	}

	// Continuous eye blink animation
	function startEyeBlink() {
		const blinkTimeline = new TimelineMax({repeat: -1, repeatDelay: 3});
		blinkTimeline
			.to(eyel, 0.1, {scaleY: 0.1, y: 55})
			.to(eyer, 0.1, {scaleY: 0.1, y: 55}, "-=0.1")
			.to(eyel, 0.1, {scaleY: 1, y: 45}, "+=0.1")
			.to(eyer, 0.1, {scaleY: 1, y: 45}, "-=0.1");
	}
	...
	function initAni(){
		...
		// Add loading text animations to match cat movement
		tl.to(loadingText, 0.2, {opacity: 0.7, y: -5}, "twink")
		  .to(loadingText, 0.1, {opacity: 1, y: 0}, "twink +=0.1")
		  .to(loadingText, 0.3, {opacity: 0.8, y: -3}, "jump")
		  .to(loadingText, 0.5, {opacity: 0, y: -20}, "jump+=0.3");
	...
	function logoVisible(){
		TweenMax.to(".logoani", 1, {opacity: 0, y: -50, ease: Power2.easeOut});
		TweenMax.to(".cat-wrapper", 1, {opacity: 0, y: -50, ease: Power2.easeOut});
		TweenMax.to(".loading-text", 1, {opacity: 0, y: -50, ease: Power2.easeOut});
		TweenMax.to(".layout", 1, {opacity: 1, ease: Power2.easeOut});
		TweenMax.to(".continue-btn", 1, {opacity: 0, ease: Power2.easeOut});
		document.querySelector('.layout').classList.remove('notvisible');
		document.querySelector('.continue-btn').classList.add('hidden');
		document.querySelector('.logoani').classList.add('hidden');
	}

	function logonotVisible(){
		TweenMax.to(".logoani", 1, {opacity: 1, y: 0, ease: Power2.easeOut});
		TweenMax.to(".cat-wrapper", 1, {opacity: 1, y: 0, ease: Power2.easeOut});
		TweenMax.to(".loading-text", 1, {opacity: 1, y: 0, ease: Power2.easeOut});
		TweenMax.to(".layout", 1, {opacity: 0, ease: Power2.easeOut});
		TweenMax.to(".continue-btn", 1, {opacity: 1, ease: Power2.easeOut});
		document.querySelector('.layout').classList.add('notvisible');
		document.querySelector('.continue-btn').classList.remove('hidden');
		document.querySelector('.logoani').classList.remove('hidden');
	}
	function resetit(){
	  TweenMax.set([head, eyes, ears], {y:20, x:30});
	  TweenMax.set(backcircle, {y:35, x:40});
	...
	// Start the continuous eye blink animation
	startEyeBlink();
